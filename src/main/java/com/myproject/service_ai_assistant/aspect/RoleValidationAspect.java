...
public class RoleValidationAspect {
    ...
    public void validateRole(JoinPoint joinPoint) {
        ...
        if (attributes == null) throw new BusinessException(ResultCode.UNAUTHORIZED);
        ...
        if (token == null || token.isEmpty()) throw new BusinessException(ResultCode.UNAUTHORIZED);

        // 4. 从 Redis 获取用户信息（格式：userId:tenantId:roleLevel）
        String userInfo = redisTemplate.opsForValue().get("token:" + token);
        if (userInfo == null) throw new BusinessException(ResultCode.TOKEN_EXPIRED);
        
        // 5. 解析用户信息
        String[] parts = userInfo.split(":");
        if (parts.length != 3) throw new BusinessException(ResultCode.UNAUTHORIZED);
        
        Long tenantId = Long.parseLong(parts[1]);
        int roleLevel = Integer.parseInt(parts[2]);
        
        // 6. 运营商（tenant_id=0）拥有所有权限，直接通过
        if (tenantId == LevelCode.ROLE_LEVEL_TENANT_ID) return;
